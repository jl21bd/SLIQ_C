...
// structure for counting each attribute
...
{
    // allocate the structure 'result' with memory size 100
    entry* result = malloc(100);
    int i;
    // summarize each attribute depending on each attributes' value
    for (i = 0; i < 10; i++)
    {
...
void main(void)
{
    ...
    // training data table as a 2d array
    // Credit Score     Married Status  Debt Status     House Own   Credit Card
    //                         Yes = 1     True = 1       Yes = 1       Yes = 1
    //                          No = 0       No = 0        No = 0        No = 0
    double input[10][5] =
    {
     {302,1,1,0,0},
    ...
    double gini_debt= ((prob_debt_y) * (1 - (pow(prob_debt_y_credit_c_y, 2)
           + pow(prob_debt_y_credit_c_n, 2)))) + ((prob_debt_n) *
           (1 - (pow(prob_debt_n_credit_c_y, 2) + pow(prob_debt_n_credit_c_n, 2))));
    ...
    // integers and char to represents the decimal points for each gini index
    int a, b, c, d, e, f;
    char A, B, C, D, E, F;
